      ...
      >
        <div className="inner-wrap flex flex-col">
          ...
          >
            {gameX.name}
            <div
              className="absolute top-12 right-0 w-full flex flex-col justify-start z-[1000] items-center "
              style={style1}
            >
              <div
                className="w-[95%] max-w-[1170px]  rounded-md border bg-[#0C1118]   overflow-hidden m-1"
                onMouseLeave={(e) => {
              ...
              >
                {gamesArray.map((item, index) => {
                  return (
                    <h3
                      key={`games__${index}`}
                      className="leading-4 py-2 hover:text-white hover:bg-purple-300 active:text-white active:bg-purple-400   focus:outline-none focus:ring focus:ring-purple-300"
                    >
      ...
      <div className="containerDescription">
        ...
        <div className="hideOnSmall">
          ...
          <Link href={'/certificate'}>
            <button className="btnBlue">Подарите сертификат</button>
          </Link>
        </div>
      </div>
